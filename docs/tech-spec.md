...
- 严格遵循用户约束：
  - 仅 TypeScript；
  - 仅使用 LLM 生成多语言场景下的 feature slug（禁止本地 slugify 降级）；
  - 配置文件为 JSON；
  - 发布形态为 `npm i -g` 全局安装；
...
- 若达到上限仍失败，报错并退出，不做本地降级与手工 `--slug` 覆盖（MVP）。
...
- LLM 不稳定或配额不足 → 限定温度 0、严格 Prompt、带原因重试、明确错误提示；不做本地降级（符合约束）。
